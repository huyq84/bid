...
// 先录入 2 条木工事件，让查询有数据
console.log('  (先录入 2 条事件作为测试数据)');
await callChat('1号楼木工3人完成大堂龙骨安装80%');
await callChat('2号楼木工2人完成吊顶龙骨50%');

const r1 = await callChat('今天木工都做了哪些事？请用表格列出来');
console.log('  reply 前 300 字:', r1.reply?.slice(0, 300).replace(/\n/g, ' / '));
...
check('LLM 自主完成"先查后改"（提到事件 ID 或生成了 action）', r3.reply?.includes('E') || r3.actions?.length >= 1);
check('reply 提到具体事件 ID（证明调了 queryEvents）', r3.reply?.match(/E\d{6,}/));
