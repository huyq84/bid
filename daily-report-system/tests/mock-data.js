...
function getPage05Photos(projectId, weekStart, weekEnd, maxCount = 6) {
  ...
  weekEvents.forEach(e => {
    ...
    (e.photos || []).forEach(p => {
      ...
      byArea[areaId].push({
        id: p.id, caption: p.caption || '现场照片',
        area: p.area || e.areaId || '', eventType: e.type, eventId: e.id,
        date: e.date, data: p.data || '', showInReport: p.showInReport !== false
      });
    });
  });
  const AREAS_SRC = MD.AREAS || AREAS;
...
function getPage06Data(projectId, weekStart, weekEnd, mode = 'fixed', displayField = 'tradeName', unit = 'people') {
  // 取表头：DB 优先，兜底 DEFAULT_STANDARD_TRADES
  ...
  // 把任意工种名归一化到 mapFrom（mapFrom 或 tradeName 任一匹配）
  ...
  weekEvents.forEach(e => {
    const list = e.payload?.laborRequirements || [];
    list.forEach(item => {
  ...
  // 动态生成模式：mapFrom 或 tradeName 命中即显示
...
function getPage07Data(projectId) {
  ...
  if (manual && manual.total > 0) {
    return {
      total: manual.total, closed: manual.closed,
      closing: manual.closing, open: manual.open, rate: manual.rate
    };
  }
  // 兜底：从 ECC_ITEMS 自动汇总
  // 关键：从 window.MockData 读取（init 时会被后端 /api/data/all 覆盖为 PostgreSQL 数据）
  ...
  areaOrder.forEach(area => {
    ...
    group.forEach(r => {
      ...
      result.push({
        ...
        schedule,
        labor: labor || '',
        material: (plan && plan.materials && plan.materials.length > 0) ? plan.materials.join('；') : '待定'
      });
    });
  });
...
window.MockData = {
  ...
  getPage06Data, getPage07Data, getPage08Data, getPage09Data, getPage10Data, getPageSectionsData, getPage12Data,
