...
function getPage0301Data(projectId) {
  ...
  plans.forEach(function (p) {
    ...
    if (p.nodeType === '关键节点') {
      ...
      if (categories[p.category].keyNodes[p.targetMonth]) {
        ...
        categories[p.category].keyNodes[p.targetMonth] = desc;
      }
    }
  ...
  return { year: year, months: months, categories: categories };
}
...
function getMilestoneData() {
  ...
  plans.forEach(function (p) {
    ...
    if (p.areaLabel) desc = (p.areaLabel + '：' + desc);
    // 检查是否已有同级同类节点行
    var existing = catMap[p.category][p.nodeType === '关键节点' ? 'keyRows' : 'subRows'].find(function (r) {
    ...
    // 子项
    if (p.subItems) {
      var subRow = catMap[p.category].subRows.find(function (r) { return r.row === '次要节点' && r.major === p.category; });
      if (!subRow) {
        subRow = { major: p.category, row: '次要节点', rowType: 'sub' };
  ...
  Object.keys(catMap).forEach(function (cat) {
    catMap[cat].keyRows.forEach(function (r) { rows.push(r); });
    catMap[cat].subRows.forEach(function (r) { rows.push(r); });
  });
...
function getPageSectionsData(projectId) {
  const zones = CONSTRUCTION_ZONE_SCHEDULES[projectId] || [];
  return zones.map(z => ({
    name: z.name || '施工段',
    items: (z.images || []).map(img => ({ image: img.src || img, label: img.label || '' })),
    floorHeaders: [
      { name: '一层', color: '#f4b084' },
      { name: '二层', color: '#a9d08e' }
    ],
    rows: (z.rows || []).length > 0 ? z.rows : [
      { building: z.id, location: '主体', process: z.name + '施工', floors: [
...
function getWeekAttendanceStats(weekStart, weekEnd, projectId) {
  ...
  if (weekStart && weekEnd) {
    ...
    while (s <= e) {
      ...
      var d = String(s.getDate()).padStart(2, '0');
      days.push(y + '-' + m + '-' + d);
      s.setDate(s.getDate() + 1);
    }
  }
  var totalDays = days.length || 5;
  return team.map(function (m) {
    ...
    return {
      ...
      phone: m.phone || '-',
      fullAttendance: absentDays.length === 0,
      totalDays: totalDays,
      presentDays: presentCount,
      absentReasons: absentDays
    };
  });
}
...
window.MockData = {
  ...
  CONSTRUCTION_ZONE_SCHEDULES, TODAY, DAILY_ATTENDANCE,
